...
> 上述脱节项已在迭代5 全部补齐（iservice.h 落地、plugin-development.md 已写、注册表管理全部服务、健康检查全实现），见 HANDOVER.md §2。

### 1.2 IService 接口（iservice.h，已落地文档 v1 并扩展）
...
### 1.3 各 service 健康度实现（10 个 QObject service）✅ 已实现
...
### 1.5 Translex_sdk CMake 目标 ✅ 已实现

- 导出公共头：`iservice.h` / `itranslationbackend.h` / `itranslationplugin.h` / `serviceregistry.h`
- 供示例插件与第三方链接

### 1.6 main_qml.cpp 重构 ✅ 已实现
...
│编辑│ 章节|批注|历史   │ (NoStack 切换重建)     │
│设置│ 图标栏按钮切换    │                        │
│可收起│ 可收起           │                        │
└───┴──────────────┴──────────────────────────┘
```

- **图标栏**（最左，Outlook 式）：44px 垂直图标（编辑/设置页面切换 + service 侧边栏面板入口），当前页主题色指示条，顶部按钮可收起/展开
- **侧边栏**（中间）：`QtQuick.Controls SplitView` 拖拽分割（180-400px），**经最左图标栏 service 按钮切换**（`serviceRegistry.sidebarPanels()` 动态注册，`Loader` 加载面板 QML）；仅含**章节/批注/翻译历史**三个面板（`qml/panels/`，无术语表面板）；宽度持久化（`ui.leftPanelWidth` 进 ui.json schema），可收起/展开
...
- 漂移修复：README/ARCHITECTURE 测试目标数 13→16、功能清单补迭代4/4b ✅
...
- 每阶段独立提交；MSVC + clang 各 16/16 全绿（16 目标，含新增 tst_registry）
