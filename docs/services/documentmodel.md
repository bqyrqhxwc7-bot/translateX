...
> 状态：已实现
> 类型：L1 内置服务（编译进应用）
> 生命周期（2026-08-13）：**应用级单例**——由 `main_qml.cpp` 创建并经 `contextProperty` 暴露。
> NoStack 模式每次导航重建页面，若模型在页面内创建会随页面销毁导致未保存编辑丢失，
> 故提升到应用级：内容/撤销历史跨页面保留；页面 `onCompleted` 仅首次（`lineCount==0`）加载示例文档。
